...
| `id` | `uuid` PK | |
| `organization_id` | `uuid` FK | |
| `order_number` | `integer` NOT NULL | random 6-digit, unique per Organization (`orders_organization_order_number_unique`) — what's actually said aloud or typed to find an order, not `id`. Drawn by `pickOrderNumber` in `services/orders.ts`; deliberately not sequential, so two of them can't be compared to guess how many orders this Store has ever placed |
